...
class ChartTurnManager:
    ...
    def __init__(self):
        ...
        self._session_has_actionable: Dict[str, bool] = {}
    ...
    def reset_turn_state(self, session_id: str) -> None:
        ...
        self._session_has_actionable[session_id] = False

    def mark_actionable_operation(self, session_id: str) -> None:
        """Mark that an actionable operation occurred in this turn."""
        self._session_has_actionable[session_id] = True
    ...
    def register_pending_chart_emission(
        ...
        # Check if any tool result in this turn was actionable
        if self._session_has_actionable.get(session_id, False):
            logger.warning(
                f"[{session_id}] Silently rejecting chart '{component_data.get('props', {}).get('title', 'Unknown')}' - "
                f"actionable operation detected in current turn"
            )
            return
...
def mark_actionable_operation(session_id: str) -> None:
    """Mark that an actionable operation occurred in this turn."""
    _default_chart_turn_manager.mark_actionable_operation(session_id)
