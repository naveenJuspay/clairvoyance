...
async def _make_genius_api_request(
...
def get_average_ticket_payment_wise_by_time(
    ...
    logger.info(
        f"Fetching real-time average ticket size with params: {params.arguments}"
    )
    payload_details = {
        "dimensions": ["payment_method_type"],
        "domain": "kvorders",
...
async def create_euler_offer(params: FunctionCallParams):
    ...
    try:
        ...
        if not merchant_id:
            await params.result_callback(
                {
                    "error": "Merchant ID not available in session context. Cannot create offer."
                }
            )
            return
        ...
        if not euler_token:
            await params.result_callback(
                {"error": "Authentication token is missing. Cannot create offer."}
            )
            return
        ...
        logger.info(
            f"Creating Euler offer with code '{offer_code}' for merchant '{merchant_id}'"
        )
        ...
        async with create_http_client(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                if response_data.get("list") and len(response_data["list"]) > 0:
                    ...
                    logger.info(
                        f"Found offer ID '{offer_id}' for offer code '{offer_code}' with complete details"
                    )
                    return offer_data
                else:
...
async def delete_euler_offer(params: FunctionCallParams):
    ...
    try:
        ...
        if not merchant_id:
            await params.result_callback(
                {
                    "error": "Merchant ID not available in session context. Cannot delete offer."
                }
            )
            return

        logger.info(
            f"Attempting to delete Euler offer with code '{offer_code}' for merchant '{merchant_id}'"
        )
        ...
        async with create_http_client(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Successfully deleted offer '{offer_code}' (ID: {offer_id})"
                )
                await params.result_callback({"data": json.dumps(success_result)})
            else:
                error_text = response.text
...
async def update_euler_offer(params: FunctionCallParams):
    ...
    try:
        ...
        if not euler_token:
            await params.result_callback(
                {"error": "Authentication token is missing. Cannot update offer."}
            )
            return
        ...
        if not merchant_id:
            await params.result_callback(
                {
                    "error": "Merchant ID not available in session context. Cannot update offer."
                }
            )
            return

        logger.info(
            f"Attempting to update Euler offer details for '{offer_code}' for merchant '{merchant_id}'"
        )
        ...
        logger.info(
            f"Using offer data from search for '{offer_code}' (avoiding detailed API call)"
        )
        ...
        if discount_value is not None:
            ...
            if existing_calc_type == "PERCENTAGE":
                ...
                if offer_description is None:
                    offer_description = (
                        f"Enjoy {discount_value}% discount on your order"
                    )
            else:  # ABSOLUTE
                if offer_title is None:
                    offer_title = f"Get ₹{discount_value} Off"
            ...
            logger.info(
                f"Auto-updated title to '{offer_title}' and description to '{offer_description}' for discount value {discount_value}"
            )
        ...
        if status is not None and all(param is None for param in detail_params):
            logger.info(
                f"Detected status-only update for offer '{offer_code}' to '{status}'"
            )
            ...
            async with create_http_client(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"Successfully {action} offer '{offer_code}' (ID: {offer_id})"
                    )
                    await params.result_callback({"data": json.dumps(success_result)})
                    return
                else:
        ...
        if start_date:
            try:
                ...
                await params.result_callback(
                    {
                        "error": f"Invalid start date format. Use 'YYYY-MM-DD HH:MM:SS' in IST. Error: {e}"
                    }
                )
                return
        ...
        if end_date:
            try:
                ...
                await params.result_callback(
                    {
                        "error": f"Invalid end date format. Use 'YYYY-MM-DD HH:MM:SS' in IST. Error: {e}"
                    }
                )
                return
        ...
        api_payload = {
            ...
            "rule_dsl": {
                "order": {
                    "max_quantity": existing_offer.get("rule_dsl", {})
                    .get("order", {})
                    .get("max_quantity"),
                    "min_quantity": existing_offer.get("rule_dsl", {})
                    .get("order", {})
                    .get("min_quantity"),
                ...
                "payment_channel": existing_offer.get("rule_dsl", {}).get(
                    "payment_channel", []
                ),
                "benefits": [
                    {
                ...
                "filters": existing_offer.get("rule_dsl", {}).get(
                    "filters", {"blacklist": [], "whitelist": []}
                ),
            },
            "status": (
            ...
            "metadata": {
                ...
                "cardResetPeriodType": existing_offer.get("metadata", {}).get(
                    "cardResetPeriodType", "offerPeriod"
                ),
                "productCustomerResetPeriodType": existing_offer.get(
                    "metadata", {}
                ).get("productCustomerResetPeriodType", "offerPeriod"),
                "productCardResetPeriodType": existing_offer.get("metadata", {}).get(
                    "productCardResetPeriodType", "offerPeriod"
                ),
                "upiResetPeriodType": existing_offer.get("metadata", {}).get(
                    "upiResetPeriodType", "offerPeriod"
                ),
            ...
            "minOfferBreakupCheckbox": existing_offer.get(
                "minOfferBreakupCheckbox", False
            ),
        ...
        logger.info(
            f"Updating offer details with ID '{offer_id}' using endpoint: {update_endpoint}"
        )
        ...
        async with create_http_client(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Successfully updated offer '{offer_code}' (ID: {offer_id})"
                )
                await params.result_callback({"data": json.dumps(success_result)})
            else:
                error_text = response.text
    ...
    properties={
        **time_input_schema["properties"],
        "dimension": {
            ...
            "enum": [
                "payment_gateway",
                "payment_instrument_overview",
                "payment_instrument_breakdown",
            ],
        },
    },
